...
export default function OrangeBirchTree(props) {
    const group = useRef();
    const { nodes, materials } = useGLTF('/assets.gltf');
    const pos = {
        x: props.position[0],
        y: props.position[1] + 10 - 4*(props.size === "small"),
        z: props.position[2]
    };
    const rot = {
    ...
    return (
        <group ref={group} dispose={null}>
            ...
            <group position={[pos.x, pos.y, pos.z]} rotation={[rot.x-Math.PI, rot.y+0.57, rot.z-Math.PI]} scale={[0.64, 0.64, 0.64]}>
                <mesh material={materials['Material.001']} geometry={nodes.mesh_10.geometry} />
            </group> :
            <group position={[pos.x, pos.y, pos.z]} rotation={[rot.x, rot.y-0.57, rot.z]} scale={[1, 1, 1]}>
                <mesh material={materials['Material.001']} geometry={nodes.mesh_9.geometry} />
            </group>}
        </group>
    );
}
