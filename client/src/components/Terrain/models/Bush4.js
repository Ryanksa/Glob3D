...
export default function Bush4(props) {
  const group = useRef();
  const { nodes, materials } = useGLTF('/assets.gltf');
  const pos = {
    x: props.position[0],
    y: props.position[1]+1.25,
    z: props.position[2]
  };
  return (
    <group ref={group} dispose={null}>
      <group position={[pos.x, pos.y, pos.z]} rotation={[0, -0.57, 0]} scale={[0.25, 0.25, 0.25]}>
        <mesh material={materials['Material.001']} geometry={nodes.mesh_22.geometry} />
      </group>
    </group>
  );
}
