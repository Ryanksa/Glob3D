...
export default function OrangeOakTree(props) {
  const group = useRef();
  const { nodes, materials } = useGLTF('/assets.gltf');
  const pos = {
    x: props.position[0],
    y: props.position[1] + 10.5 - 4*(props.size === "small"),
    z: props.position[2]
  };
  const rot = {
  ...
  return (
    <group ref={group} dispose={null}>
      ...
      <group position={[pos.x, pos.y, pos.z]} rotation={[rot.x-Math.PI, rot.y+0.57, rot.z-Math.PI]} scale={[0.64, 0.64, 0.64]}>
        <mesh material={materials['Material.001']} geometry={nodes.mesh_5.geometry} />
      </group> :
